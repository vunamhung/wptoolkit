...
class System_Status implements Renderable {
	...
	public function __construct($args = []) {
		$this->defaults = [
			'download_button_text' => esc_attr_x('Download', 'Download log file to your computer', 'vnh_textdomain'),
			'menu_slug' => 'diagnostic_info',
		];
		$this->args = wp_parse_args($args, $this->defaults);

		$query = ['page' => $this->args['menu_slug'], 'download-log' => true, 'nonce' => wp_create_nonce('download-log')];
	...
	public function render() {
		...
		$html .= '</textarea>';
		$html .= sprintf('<a href="%s" class="button">%s</a>', $this->debug_file_url, $this->args['download_button_text']);
		$html .= '</div>';
	...
	public function get_diagnostic_info() {
		...
		if (!empty($active_plugins)) {
			...
			foreach ($active_plugins as $plugin) {
				$active_plugins_log[1][] = $this->get_plugin_details(WP_PLUGIN_DIR . DIRECTORY_SEPARATOR . $plugin);
			}
